...
import { vmPageShell } from '../lib/vehicleManagementLayout';
import { useCreateDiesel } from '../vehicles.hooks';
...
export default function DieselConfirmationPage(): JSX.Element {
  ...
  const onSubmit = (): void => {
    create.mutate(formData, {
      onSuccess: () => {
        void message.success('Diesel details submitted');
        nav('/diesel');
      },
    });
  };
  ...
  if (Object.keys(formData).length === 0) {
    return (
      <div style={S.container}>
        <div style={S.pageHeader}>DIESEL CONFIRMATION</div>
  ...
  return (
    <div style={S.container}>
      ...
      <div style={S.body}>
        ...
        <div style={S.grid}>
          {Object.entries(formData).map(([k, v]) => (
            <div key={k}>
              ...
              {k === 'dieselSlipPhoto' && typeof v === 'string' && v.startsWith('data:image') ? (
                <img
                  src={v}
                  alt="slip"
                  style={{
        ...
        <div style={S.actions}>
          <button type="button" style={S.btnBlack} onClick={() => nav(-1)}>
            Back
          </button>
          <button
            type="button"
            style={{ ...S.btnYellow, opacity: create.isPending ? 0.7 : 1 }}
            onClick={onSubmit}
